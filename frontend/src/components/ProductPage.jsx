...
import { addToCart, getItemLocalStorage } from '../store/actions';
...
function ProductPage({ match }) {
    ...
    return (
        <section className="catalog-item">
            {state.toCart ? redirect('/cart') : null}
                    ...
                    <div className="text-center">
                        <p>Размеры в наличии:
                            {state.product.sizes ? state.product.sizes
                                .map((el, index) => el.avalible ? <span key={index} className="catalog-item-size" onClick={(event) => handlerChoiceSize(event, el.size)}>{el.size}</span> : null)
                                : null}
                        </p>
